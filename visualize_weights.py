...
def plot_tsne(features, labels_task, labels_type, title, filename_suffix):
    if len(features) < 2:
...
def main():
    ...
    for task_id in range(1, 7):
        for run_type in ["a_Task", "Task"]:
            if task_id == 1 and run_type == "a_Task":
                continue
                    ...
                    if m:
                        ...
                        if "lora_A" in k:
                            ...
                            if m_expert:
                                ...
                                is_match = False
                                if layer_idx < 31:
                                    if run_type == "a_Task" and expert_idx == 0:
                                        is_match = True
    ...
    def plot_three_variants(features, labels_task, labels_type, base_title, base_filename):
        # 1. Combined
        plot_tsne(features, labels_task, labels_type, base_title, base_filename)
        ...
        if hide_indices:
            ...
            plot_tsne(hide_features, hide_labels_task, hide_labels_type, base_title + " (HiDe)", base_filename + "_hide")
        ...
        if a_indices:
            ...
            plot_tsne(a_features, a_labels_task, a_labels_type, base_title + " (Variant A)", base_filename + "_a")

    # 1. Plot Matrix A
    print(f"Collected {len(matrices_A)} matrices for A. Expected: 384")
    plot_three_variants(matrices_A, labels_A_task, labels_A_type, "t-SNE Visualization of Matrices A", "A")
    
    # 2. Plot Matrix B
    print(f"Collected {len(matrices_B)} matrices for B. Expected: 384")
    plot_three_variants(matrices_B, labels_B_task, labels_B_type, "t-SNE Visualization of Matrices B", "B")
    
    # 3. Plot Projectors
    print(f"Collected {len(projectors)} projectors. Expected: 12")
    plot_three_variants(projectors, labels_proj_task, labels_proj_type, "t-SNE Visualization of Projectors", "projectors")
    ...
    plot_three_variants(all_anchors, all_labels_task, all_labels_type, "t-SNE Visualization of Anchors", "anchors")
